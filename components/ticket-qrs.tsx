      ...
      {tickets.map((t, i) => {
        ...
        return (
          ...
          >
            ...
            {t.ticketTypeName ? (
              <div style={{ fontSize: 18, fontWeight: 800, color: "#000", margin: "4px 0 16px" }}>
                {t.ticketTypeName}
              </div>
            ) : null}
            <div style={{ display: "inline-block", border: "3px solid #F5E642", borderRadius: 12, padding: 10, background: "#fff" }}>
              <QRCodeSVG value={t.qrToken} size={200} />
            </div>
            {t.ownerName ? (
              <div style={{ fontSize: 14, color: "#666", marginTop: 12 }}>{t.ownerName}</div>
            ) : null}
            <div style={{ fontSize: 11, letterSpacing: 1, fontWeight: 700, color: used ? "#999" : "#000", marginTop: 8 }}>
